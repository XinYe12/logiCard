...
namespace LogiCard.Board
{
    ...
    internal static class BoardSurfaceMaterials
    {
        ...
        // 2026-08-12 playtest: terracotta asphalt read as farm soil. Urban Yard = cool grey asphalt,
        // lightly wet, no warm dirt tint. Tile kept high for ortho 2.6-3.4 grain.
        public static Material YardFloor => _yard ??= BuildWetSurface(
            "asphalt_diff", "asphalt_nor", "asphalt_rough",
            fallback: new Color(0.32f, 0.32f, 0.34f),
            wetSmoothnessBoost: 0.28f,
            tint: new Color(0.78f, 0.80f, 0.84f),
            tile: 3.4f);

        // 2026-08-12: stop using nappin (Mat)Floor_URP for Hall — featureless brown slab read as farm dirt
        // under the camera. Interior Hall = Poly Haven wood planks (city interior floor).
        public static Material HallFloor => _hall ??= BuildWetSurface(
            "wood_diff", "wood_nor", "wood_rough",
            fallback: new Color(0.62f, 0.48f, 0.32f),
            wetSmoothnessBoost: 0.22f,
            tint: new Color(1f, 0.92f, 0.78f),
            tile: 2.2f);

        // 2026-08-12: Vault = bright polished concrete / marble read (cool light grey), not brass mud.
        public static Material VaultFloor => _vault ??= BuildWetSurface(
            "concrete_diff", "concrete_nor", "concrete_rough",
            fallback: new Color(0.72f, 0.74f, 0.78f),
            wetSmoothnessBoost: 0.48f,
            tint: new Color(0.92f, 0.94f, 0.98f),
            tile: 2.0f);

        // Match Yard: cool urban asphalt approach lanes (not ochre dirt).
        public static Material FlankFloor => _flank ??= BuildWetSurface(
            "asphalt_diff", "asphalt_nor", "asphalt_rough",
            fallback: new Color(0.30f, 0.30f, 0.32f),
            wetSmoothnessBoost: 0.24f,
            tint: new Color(0.72f, 0.74f, 0.78f),
            tile: 3.2f);
        ...
        public static Material BrickWall => _wall ??= BuildWetSurface(
            ...
            tint: new Color(0.96f, 0.90f, 0.84f),
            tile: 1.4f);
