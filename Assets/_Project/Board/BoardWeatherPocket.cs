...
namespace LogiCard.Board
{
    ...
    public sealed class BoardWeatherPocket : MonoBehaviour
    {
        ...
        /// children (<c>Weather_Fair</c> / <c>Weather_Storm</c> / <c>Weather_Sunny</c>) so a future
        /// card only mounts that module. Sunny = Sunshine / 万里无云 — cloudless, bright mood lighting
        /// (Atmosphere-owned). Same-mood calls are a no-op (defense in depth — <c>RoundPlayback</c>
        /// also guards the call site; this API is driven from a per-tick Playback presenter, so it
