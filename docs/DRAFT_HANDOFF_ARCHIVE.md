# Draft Handoff — Archive

Entries moved out of `DRAFT_HANDOFF.md` to keep that file's "read first" cost down. Nothing here was
deleted; git history has the exact same record regardless, and this file preserves the doc-level
narrative for anyone who wants the story rather than just the commit log. Two archival passes so far,
newest first:

- **2026-08-10 (second pass):** `feat/character-model-rework` through the 2026-08-09 Phase-1/board-merge
  entries — moved when the main file hit 688 lines again after a long single-day autonomous push
  (checkpoints, URP, reflections, clouds, glass, C55/C56). `PRODUCT_MEMORY.md`'s `C52`–`C56` rows and
  `SCHEDULE.md`'s Phase 5 status are the current authoritative summary of what this batch covers.
- **2026-08-10 (first pass):** the full C46–C51 scope-pivot narrative, C45 board rework, pawn-art rework,
  Day 9–12 ship history, and 2026-08-06 door/block fixes — moved when the main file first hit 680 lines.
  `PRODUCT_MEMORY.md`'s `C46`–`C51` rows are the authoritative summary of that batch.

If you're trying to answer "what's landed vs. still open right now," read `DRAFT_HANDOFF.md` (current)
first, then `PRODUCT_MEMORY.md`'s Binding ID table — you shouldn't need this file for that. Read this file
only if you want the session-by-session story behind an old decision.
...
Scout/Juggernaut pawn art, and the landscape HUD/app-flow shell.
...
- `12f8a02` — **Lighting/post-processing fix**, in response to human feedback that Day 9 read as "too plain and dull" despite the path/cardstock/grid work landing:
  ...
  - *(Historical note, since superseded: this row wired in URP's default `PostProcessData` and a `BuildDioramaVolume` global Volume back on 2026-08-07. By 2026-08-10 that Volume Profile had gone missing/unset again — see current `DRAFT_HANDOFF.md`'s URP audit finding, since fixed via `feat/urp-post-processing`.)*
...
## Still unfinished (as of 2026-08-08 — see current DRAFT_HANDOFF.md for what's open today)
...
## Tomorrow / next agent — Wave 3 kickoff (as of 2026-08-08; superseded by the pivot and current wave)
...
## Blockers / notes (historical — main project path has since moved to Windows, `D:\projects\Game\logiCard`)

- Unity **6000.5.5f1**; main project `/Users/xuxinye/Documents/projects/Game/LogiCard` *(Mac path — session moved to a Windows machine afterward; current path is `D:\projects\Game\logiCard`)*. Editor is typically open there (user playtests live) — batchmode needs a different worktree path. Spin up a disposable one (`git worktree add`) and remove it after.
...
- `master` tracks `origin/master`; everything above is local-only, not pushed *(historical — confirm current push status if this matters again)*.
