# Draft Handoff — Archive (pre-2026-08-09)

Entries moved out of `DRAFT_HANDOFF.md` on 2026-08-10 to keep that file's "read this first" cost down —
it had grown to 680 lines, most of it multi-day-old ship history from before the current look-and-feel +
UI wave. Nothing here was deleted; git history has the exact same record regardless, and this file
preserves the doc-level narrative for anyone who wants the story rather than just the commit log.

**What superseded most of this content's practical value:** `PRODUCT_MEMORY.md`'s binding `C46`–`C51` rows
are now the authoritative summary of the pivot narrated in the first section below; `SCHEDULE.md` tracks
which days/phases are done. If you're trying to answer "what's landed vs. still open right now," read
`DRAFT_HANDOFF.md` (current) first, then `PRODUCT_MEMORY.md`'s Binding ID table — you shouldn't need this
file for that. Read this file only if you want the session-by-session story behind an old decision.
...
- `12f8a02` — **Lighting/post-processing fix**, in response to human feedback that Day 9 read as "too plain and dull" despite the path/cardstock/grid work landing:
  ...
  - *(Historical note, since superseded: this row wired in URP's default `PostProcessData` and a `BuildDioramaVolume` global Volume back on 2026-08-07. By 2026-08-10 that Volume Profile had gone missing/unset again — see current `DRAFT_HANDOFF.md`'s URP audit finding. Not yet understood why; worth checking whether a later change cleared `m_VolumeProfile` or whether it was a different Volume object than `BuildDioramaVolume` creates.)*
