...
(`f45e986`); otherwise 2026-08-15 agent pass added recommendations for every open row (A3–A5,
B1–B9, C2–C7, D1–D6), still waiting on human Decision column. Char Select UI/motion carousel work
earlier in this branch's history was handed to **UI** dept partway through (see Mandate below) and
remains mergeable history but is no longer this seat's active scope. See the Rebase entry under
Done for what this pass touched.
...
- Decision-sheet walk with human — **paused at A3**, still needs human Decisions. Partial answers on the sheet:
  - **C1 = C** (FF = resolve-time booking only)
  - **A1 = Bomber first**
  - **A2 = hardcoded archetype→verb map**
- 2026-08-15: agent legwork pass added an **Agent recommendation** column across every still-open
  row (A3–A5, B1–B9, C2–C7, D1–D6) so the human's resume pass is pick-one-or-override instead of a
  cold read. Nothing promoted; Decision column untouched by the agent.

## Waiting on human (when ready)

1. Resume [`CHARACTER_DECISION_SHEET.md`](CHARACTER_DECISION_SHEET.md) from **A3** through A5, Part B, rest of Part C, Part D — agent recommendations are there to accept or override, not a substitute for the Decision column.
...
- Resume “walk the sheet” with simple-language prompts.
- No code until sheet done + Integrator contracts a slice.
- Commit dirty decision-sheet (+ STATUS / handoff) when you ask.

## Integrator ask (unchanged until sheet done)

Sync PARALLEL_OPS / INDEX / GDD §11 for Char Select → UI handoff; after answers land, promote to PRODUCT_MEMORY and sequence C36 before Bomber Sim.
