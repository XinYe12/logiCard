...
| **Phase 5** | Commercial Art Bar | `ART_DIRECTION.md`'s raised bar met for the shipped roster — placeholder Quaternius meshes replaced/substantially reworked; a cold observer calls it "a real game," not "a prototype." | **Active, top priority.** Substantial progress landed 2026-08-10: env checkpoint 1 (weather/lighting), checkpoint 2 (real Poly Haven PBR board-surface textures, **C54**), checkpoint 3 (real Quaternius door/prop meshes replacing tinted boxes, **C54**); URP post-processing gap closed (Volume Profile, SSAO, MSAA, soft shadows) plus a photo-mode stretch goal; camera gained smooth right-drag rotation (`BoardCameraRig`); pawn animation moved from stepped stop-motion to smooth interpolation (**C55**) once the realism jump made the old stepped look read as a framerate bug. UI side (`feat/ui-component-system`, then `feat/ui-dock-polish`) fully shipped and merged. **Still open before calling this phase's bar met:** human screenshot/Editor sign-off on the checkpoint 2/3 work (never confirmed in-Editor), wet-surface reflections (flagged by the human as "needs more rework" — SSR isn't available in this URP version, reflection probes are the likely path), and the character models themselves — still the original Quaternius chibi import, flagged in `ART_DIRECTION.md`/`PAWN_ART_REWORK_PLAN.md` as "likely too generic/undifferentiated for a paid, distinctly-branded product," never reviewed or reworked since the C53 pivot. Core gameplay stays paused until this phase is genuinely done, not just improved. |
