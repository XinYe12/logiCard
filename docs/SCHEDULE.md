...
| **Phase 2** | Real Networking Foundation | `NETWORKING_DESIGN.md`'s transport/topology decided and its first slice built — a real, two-process, real-transport tape-synced match replacing the same-process `GhostResolver` stand-in; host-integrity question has an explicit answer. | **In progress** — transport + host-integrity locked (**C52**, 2026-08-09: custom resolve-relay, server-authoritative). First slice merged 2026-08-09: `IMatchResolver` seam + `RelayMatchResolver` + standalone relay process, proven via two-process determinism test (`Relay/LogiCard.Relay.Tests`) matching local `GhostResolver.Resolve`. **Wired live** (2026-08-09): Find Match now picks `RelayMatchResolver`, Local Play keeps `LocalMatchResolver` — automated (`AppFlowPlayModeTests`) coverage green, but the real two-process network round-trip hasn't had a human smoke test yet. **Not yet exit-criteria-complete:** wire hosting/deploy target, real matchmaking/session assignment (Find Match is still a UI stub), ranked/casual split, reconnect policy, matchmaking cost estimate, and anti-cheat audit depth are all still OPEN — see `NETWORKING_DESIGN.md`'s OPEN summary. |
