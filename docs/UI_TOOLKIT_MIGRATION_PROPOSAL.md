# UI Toolkit migration â€” proposal (fed by the Character Select pilot)
...
Integrator-facing writeup that request needs before it can be scheduled â€” see `docs/PARALLEL_OPS.md`
...
that verdict with real, not speculative, evidence â€” the pilot existed, compiled, and passed its
PlayMode coverage.

**Update, same day:** the pilot code has since been **reverted** â€” it never had a human visual Play
...
color â€” see `Assets/_Project/Art/UI/THIRD_PARTY.md`). The pilot code and its fixes are preserved in
git history at `2c99a08` on `feat/char-select-motion` if this gets revisited. **Everything below this
point is unchanged** â€” the findings are real regardless of whether the pilot code is currently live,
and remain the impact/risk data for whoever next considers this migration.

## Bottom line

UI Toolkit is a legitimate "mature" choice â€” it's Unity's own actively-developed UI system, not a
...
It does **not** prove the *in-match HUD* should â€” `UI_TOOLS_RESEARCH.md`'s original caution about
...
hand-styled inline (no `.uxml`/`.uss` asset files â€” same "no external asset" posture `UiStyle.RoundSprite`
...
- 2-item center/flank carousel, ~650ms coordinated crossfade, driven by the *same*, completely
  unmodified `UiMotion.cs` coroutine helper â€” the tween layer is UI-technology-agnostic and needs no
...
- `ConfirmCharacter` stays a uGUI `Button`, built by `AppFlowController.BuildCharacterSelect` exactly
  as before â€” this screen is deliberately **hybrid** (Toolkit carousel + uGUI sibling button), because
  that's the realistic rollout shape, not a green-field rebuild.

Verified via batchmode only (see Verification below) â€” **no interactive/visual Play pass has happened
...
## Real findings (not speculation â€” hit these bugs, fixed them, kept the fix)
...
1. **`UIDocument.rootVisualElement` is torn down and rebuilt empty on every `OnEnable`/`OnDisable`.**
   Confirmed Unity behavior, not a bug â€” Unity's own guidance is to either rebuild on every re-enable or
   avoid disabling the GameObject at all. `AppFlowController` hides every pre-match screen by
   `GameObject.SetActive(false)` (`Show()`'s `SetActive` calls) â€” the naive port (`UIDocument` directly
...
2. **Parent cascades hit the same bug one level up.** The first fix (host the `UIDocument` on a sibling
   ...
   `AppFlowController`'s shared `_root` panel â€” which itself gets `SetActive(false)` whenever the whole
   ...
   **Fix:** the `UIDocument` host must be parented under something a full rollout is *certain* never
   gets disabled â€” in this pilot, that's "nothing" (scene root) â€” which raises finding 3.
3. **A scene-root, nothing-ever-disables-it host also means nothing ever destroys it.** Orphaned
   ...
   query can silently match the *stale* one instead of the live one â€” the click lands, but on a dead
   ...
   its `UIDocument` host in `OnDestroy` â€” ownership can't be inferred from Transform parenting the way
...
4. **Cross-technology click simulation needs a different pattern than uGUI's tests use.**
   `Button.onClick.Invoke()` (uGUI, a public `UnityEvent`) has no Toolkit equivalent â€” `Button.clicked`
   ...
   via `element.SendEvent(ClickEvent.GetPooled())` â€” this goes through the real event dispatcher, so it
...
5. **Missing theme stylesheet â€” a real, currently-unresolved gap.** Every batchmode run logs
   ...
   "no external asset, batchmode-safe" posture â€” but that means no default runtime theme is attached
   ...
   one. **This is exactly the gap the "no visual Play pass yet" caveat above is about** â€” a full rollout
...
6. **Assembly references.** `UnityEngine.UIElementsModule` had to be added to both
   ...
   â€” not automatic just because the built-in module is present in `Packages/manifest.json`
...
7. **Cross-technology draw order is a real tuning knob, not automatic.** `PanelSettings.sortingOrder`
   ...
   (`sortingOrder = 10f` here) â€” worth a second look in the human Play pass, since it was chosen by
...
8. **Layout math ports mechanically, which is a genuine upside.** uGUI's `anchorMin`/`anchorMax`
   ...
   percentages (flip Y, since uGUI is Y-up and USS is Y-down) â€” `CharacterSelectView.StretchVe` is a
   ...
   `transformOrigin` for the pivot-anchored grow/shrink â€” every layout number in the Toolkit version is
...
9. **Native rounded corners, no baked sprite.** `UiStyle.RoundSprite` exists specifically to work around
   ...
   `AssetDatabase.GetBuiltinExtraResource`). USS `border-*-radius` is native and needs no baked texture â€”
   a small but real simplification a rollout gets "for free" once a screen is on Toolkit.

## Recommended sequencing

Not a decision â€” a proposed order for the Integrator/human to confirm or amend:
...
2. **If the pilot holds up visually, migrate one more shell screen** (Map Select or Lobby â€” both
   simpler than Character Select, no crossfade) to confirm the workaround pattern (findings 1â€“4) holds
...
3. **Leave `ProgramHud` / in-match HUD / board-anchored prompts on uGUI.** Nothing in this pilot
   ...
   revisited â€” don't fold it into "the UI Toolkit migration" as if it's the same-sized problem.
4. **`ModalDialog` needs its own sign-off**, not a unilateral touch from whichever screen gets to it
   first â€” it's owned by the sibling `logiCard-modal-restyle` worktree per
...
5. **Resolve the theme stylesheet question once, centrally**, before a second screen migrates â€” better
...
- `BootThroughLobbyLocalPlayReachesMatchHud` (the full Bootâ†’Lobbyâ†’Match click-through, unchanged in
  scope) and `CharacterSelectNextRotatesArchetypeAfterCrossfade` both cover the Toolkit carousel via the
  new `FindVisualElement`/`ClickVisualElement` helpers â€” same coverage shape as before, adapted to the
  new element type.
- **Not done:** an actual human Play pass. See finding 5 and sequencing step 1 â€” this is the load-bearing
